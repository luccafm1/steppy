...
boolean colisaoDetectar, jogoIniciado = false;
...
public void setup(){
  ...
  //vetores camera
  playerPos = new PVector(random(4000,30000),random(4000,30000),1200);
  playerVel = 30;
...
public boolean colisao(PVector x, PVector y, PVector z) {
  float px = playerPos.x;
  float py = playerPos.y;
  float pz = playerPos.z;

  float minX = min(x.x, y.x, z.x);
  float maxX = max(x.x, y.x, z.x);
  float minY = min(x.y, y.y, z.y);
  float maxY = max(x.y, y.y, z.y);
  float minZ = min(x.z, y.z, z.z);
  float maxZ = max(x.z, y.z, z.z);

  if (px >= minX && px <= maxX && py >= minY && py <= maxY && pz >= minZ && pz <= maxZ) {
    // Collision occurred
    float distX = min(abs(px - minX), abs(px - maxX));
    float distY = min(abs(py - minY), abs(py - maxY));
    float distZ = min(abs(pz - minZ), abs(pz - maxZ));

    // Find the closest face to the player
    if (distX < distY && distX < distZ) {
      // Collided with the X face
      if (px < minX) {
        playerPos.x = minX;
      } else {
        playerPos.x = maxX;
...
public void bloco(float blocoX, float blocoY, float comprimento, float altura){
  PVector[] verticesParede = new PVector[16];
    for (int i = 0; i < comprimento; i += comprimento/2) { 
      ...
      pushMatrix();
      translate(verticesParede[0].x,verticesParede[0].y,verticesParede[0].z);
      box(100);
      popMatrix();
      pushMatrix();
      translate(verticesParede[1].x,verticesParede[1].y,verticesParede[1].z);
      box(100);
      popMatrix();
      pushMatrix();
      translate(verticesParede[4].x,verticesParede[4].y,verticesParede[4].z);
      box(100);
      popMatrix();
      pushMatrix();
      translate(verticesParede[5].x,verticesParede[5].y,verticesParede[5].z);
      box(100);
      popMatrix();
      //pushMatrix();
      //translate(verticesParede[8].x,verticesParede[8].y,verticesParede[8].z);
      //box(100);
      //popMatrix();
      //pushMatrix();
      //translate(verticesParede[9].x,verticesParede[9].y,verticesParede[9].z);
      //box(100);
      //popMatrix();
      //pushMatrix();
      ...
      //popMatrix();
      //pushMatrix();
      //translate(verticesParede[13].x,verticesParede[13].y,verticesParede[13].z);
      //box(100);
      //popMatrix();
    }
}
